...
public class DroneInputHandler {
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        if (FpvOverlay.fpvActive) {
            ...
            float pitchIn = (float) (dy * 0.6);
            float rollIn = (float) (dx * 0.6);

            float yawIn = 0f;
            if (mc.options.keyLeft.isDown()) yawIn += 1.5f;
            if (mc.options.keyRight.isDown()) yawIn -= 1.5f;
    ...
    private static void exitFpv(Minecraft mc) {
        if (FpvOverlay.fpvActive && lastUuid != null) {
            ModNetwork.CHANNEL.sendToServer(new DroneControlPacket(
    ...
    private static FpvDroneEntity findDrone(Minecraft mc, UUID uuid) {
        ...
        AABB area = new AABB(p.x - range, p.y - range, p.z - range, p.x + range, p.y + range, p.z + range);
        List<FpvDroneEntity> drones = mc.level.getEntitiesOfClass(FpvDroneEntity.class, area);
        for (FpvDroneEntity d : drones) {
            if (d.getUUID().equals(uuid)) return d;
        }
        return null;
    }
